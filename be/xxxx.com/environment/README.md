...
├── elk/                       
│   ├── pipeline/                ← Logstash pipeline config (input, filter, output)
│   ├── elasticsearch.yml        ← config Elasticsearch (cluster, index settings)
│   ├── logstash.yml             ← config Logstash (pipeline configs, logging)
│   └── kibana.yml               ← config Kibana (Elasticsearch URL, index patterns)
```
...
It create and start 10 containers:
...
| `pre-event-redis-exporter` | `oliver006/redis_exporter` | `9121` | Metrics Redis |
| `pre-event-mysqld-exporter` | `prom/mysqld-exporter` | `9104` | Metrics MySQL |
| `pre-event-elasticsearch` | `docker.elastic.co/elasticsearch/elasticsearch:8.13.4` | `9200, 9300` | Search & log storage |
| `pre-event-logstash` | `docker.elastic.co/logstash/logstash:8.13.4` | `5044, 5000, 9600` | Log pipeline |
| `pre-event-kibana` | `docker.elastic.co/kibana/kibana:8.13.4` | `5601` | Log visualization |
...
**FROM:** Prometheus container tự tạo ra khi bắt đầu lưu metrics.
...
**FROM:** Grafana container tự tạo ra khi khởi động.
...
## Database Seed (chạy thủ công)

`ticket_init.sql` chỉ chạy **một lần** khi container khởi tạo. Nếu bạn muốn reset dữ liệu bất kỳ lúc nào mà không cần xóa toàn bộ `data/db_data/`, dùng seed command của Spring Boot.

### Cách chạy seed

**Yêu cầu:** Docker đang chạy (MySQL + Redis đang up).

```bash
# Từ thư mục gốc của project (xxxx.com/)
mvn spring-boot:run -pl xxxx-start -Dspring-boot.run.arguments="--spring.profiles.active=seed"
```

Hoặc sau khi đã build fat jar (`mvn package`):
...
1. Xóa toàn bộ dữ liệu trong `ticket_item` và `ticket`
2. Reset `AUTO_INCREMENT` về 1
3. Insert lại dữ liệu mẫu (2 đợt mở bán, 4 loại vé)
4. Tự thoát sau khi xong

> **Khác với `ticket_init.sql`:**
>
> - `ticket_init.sql` chạy tự động 1 lần khi Docker khởi tạo, không chạy lại được nếu `data/db_data/` đã tồn tại.
> - Seed command chạy bất kỳ lúc nào theo yêu cầu, giống `npx prisma db seed`.

### Thêm dữ liệu mẫu mới

Mở file [xxxx-start/src/main/java/com/xxxx/seed/DatabaseSeeder.java](../xxxx-start/src/main/java/com/xxxx/seed/DatabaseSeeder.java), thêm method mới và gọi nó từ `run()`.
