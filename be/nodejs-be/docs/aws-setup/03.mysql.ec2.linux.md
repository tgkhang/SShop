# Hướng dẫn cài đặt MySQL trên AWS EC2 Instance (Amazon Linux 2)

> **Lưu ý**: File này hướng dẫn cài đặt MySQL Server trên **Amazon Linux 2** EC2 instance.
>
> - Nếu bạn sử dụng **Amazon Linux 2023 (AL2023)**, vui lòng xem file [03.mysql.ec2.al2023.md](./03.mysql.ec2.al2023.md)
...
search andwatch install mysql dev.mysql
...
**Cách 2: Cài đặt từ MySQL Repository chính thức (RECOMMENED)**
...
Khi cài đặt lần đầu, MySQL tự động tạo một **mật khẩu tạm thời** cho user `root` và lưu trong log file. (linix only)
...
### 6.3. Tạo User cho Remote Access
...
**Mở port 3306 trong AWS Security Group** (xem chi tiết ở file [03.mysql.ec2.al2023.md](./03.mysql.ec2.al2023.md) section 6.4)
...
```bash
mysql -h YOUR_EC2_IP -u tipjs -p
```

**⚠️ LƯU Ý:** Từ localhost trên EC2, không dùng IP public, chỉ dùng `localhost` hoặc `127.0.0.1`

---

## 7. Import dữ liệu từ file SQL

### 7.1. Chuẩn bị file SQL
...
### 7.3. Copy file SQL từ local lên EC2
...
**Bước 3:** Tạo database (nếu chưa có, file SQL có thể đã chứa lệnh này)
...
### 7.5. Import trực tiếp từ command line (Không cần vào MySQL CLI)
...
## 9. Security Configuration (Khuyến nghị)
