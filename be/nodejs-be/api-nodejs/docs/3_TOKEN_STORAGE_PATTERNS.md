...
Looking at the authenticationV2 middleware in authUtils.js, here's how it works:
...
1. Normal API Requests (Access Token Only)
   When the access token is still valid:
   Headers:
   x-client-id: <userId>
   authorization: <accessToken>
   // NO refresh token needed
2. Refresh Token Requests (When Access Token Expires)
   Only when calling /refresh-token endpoint:
   Headers:
   x-client-id: <userId>
   x-rtoken-id: <refreshToken>
   // NO access token needed
   Current Implementation Issue
   The current code checks refresh token BEFORE access token (lines 103-108), which means:
   ❌ If a refresh token header exists, it always uses refresh token flow
   ❌ This could cause issues if the client accidentally sends both headers
   ❌ The refresh token check should ideally be only for the /refresh-token endpoint
   Recommended Approaches:
   Option 1: Route-Specific Middleware (Best Practice)
   Different middleware for different endpoints:
   // For /refresh-token endpoint only
   export const authenticationRefreshToken = asyncHandler(async (req, res, next) => {
